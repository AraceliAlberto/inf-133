...
class RESTRequestHandler(BaseHTTPRequestHandler):
    ...
    # Cuerpo de la Solicitud (POST, PUT)
    def read_data(self):
    ...
    def do_PUT(self):
        if self.path.startswith("/estudiantes/"): #con que empieza "startswitch"
            id = int(self.path.split("/")[-1]) 
            estudiante = self.find_student(id) #id no es reservada cambiar nombre
            data = self.read_data()
            if estudiante:
